...
def public():
	...
	for userr in uid:
		try:
			...
			os.system('clear')
	try:
		print(f' [] ZIDDI SEHZADA ID: {P}'+str(len(id)))
		print('')
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{u}')
...
def passwrd():
	...
	print('\033[97;1m[\033[92;1m+\033[97;1m] \033[0;92mZIDDI SEHZADA IDz :\033[0;97m '+str(len(id)))
	...
	print(f'{h}[{h}💚{h}]{h} Your ZIDDI SEHZADA OK idz : {h}%s '%(ok))
	input('CLICK ENTER TO EXIT ')
		
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r{bo}[ZIDDI-SEHZADA] {P}[{h}{loop}{P}]>~<[{h}{len(id)}{P}]{bo}•{P}[{h}Ok{P}•{bo}{ok}{P}] "),
	sys.stdout.flush()
